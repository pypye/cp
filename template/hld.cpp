...
void hld(int u)
{
    ...
    for (auto v : a[u])
        if (v.F != par[u])
            if (heavy == -1 || child[v.F] > child[heavy])
                heavy = v.F;
    if (heavy > -1)
        hld(heavy);
    for (auto v : a[u])
        if (v.F != par[u] && v.F != heavy)
            ++block, hld(v.F);
    out[u] = cnt;
}
...
{
    if (l >= u && r <= v)
        return upper_bound(all(s[id]), k) - s[id].begin();
    int mid = (l + r) / 2, ans = 0;
    if (u <= mid)
        ans += get(u, v, k, l, mid, id * 2);
    if (v > mid)
        ans += get(u, v, k, mid + 1, r, id * 2 + 1);
    return ans;
}
int lca(int u, int v)
{
...
int main()
{
    ...
    while (m--)
    {
        ...
        else
        {
            ...
            int u = b[x].F, v = b[x].S;
            if (par[v] == u)
                cout << get(in[v], out[v], k) - (val[v] <= k) << '\n';
            else
                cout << get(in[1], out[1], k) - (get(in[v], out[v], k) - (val[v] <= k)) << '\n';
        }
    }
}
