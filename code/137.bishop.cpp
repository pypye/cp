...
struct bigint{
    ...
    void operator = (long long v){sign = 1;
        if(v < 0) sign = -1, v = -v;
...
int n, k;
bigint w[maxn], b[maxn], fw[maxn][maxn], fb[maxn][maxn], ans;
int main()
{
    ios_base::sync_with_stdio(0);
    cin.tie(0); cout.tie(0);
    ifstream cin("bishop.inp");
    ofstream cout("bishop.out");
    cin >> n >> k;
    for(int i = 1; i <= n; i += 2){
        w[i] = i;
        w[i+1] = i;
    ...
    FOR(i, 0, n) fw[i][0] = 1, fb[i][0] = 1;
    FOR(i, 1, n){
        FOR(j, 1, k){
            fw[i][j] = fw[i-1][j] + fw[i-1][j-1] * (w[i] - (j-1));
            fb[i][j] = fb[i-1][j] + fb[i-1][j-1] * (b[i] - (j-1));
        }
    }
    FOR(i, 0, k) ans += fw[n][i] * fb[n-1][k-i];
    cout << ans;
}
